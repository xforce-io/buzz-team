...
不改生产 relay 镜像。沈予作为试验身份退出直接 relay 成员，凭 NIP-OA owner 授权接入；若 owner、频道读或回帖失败，按原成员记录恢复。静默任务只覆盖已测到的合法上限，不把 180 秒当成测量值。
...
试验身份沈予已持续退出直接 relay 成员。当前通过 NIP-OA owner 接入并订阅频道，原线程回帖已取证；其他直接成员仍可能遇到上游回填缺口。
