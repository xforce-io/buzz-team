...
Desktop 配置作者策略。默认候选为 `owner-only`；当前版本上的 owner、同 owner 已认证身份和边界外成员须各经真实提及验证。额外作者只有明确业务依据时才使用上游 allowlist。业务写路径在本机私有策略中逐项声明；Seatbelt 拒绝其余写入。
...
当前业务协作是否需要 owner / 同 owner 身份之外的作者，须由 S1 真实消息与业务记录确认；未确认前不维护显式名单。
