...
无新增 GUI。CLI 提供成本账本创建、turn 记录、结构化报告、handoff 写入和 `context restore` 消费交接。报告分别显示预算状态和交接状态；不可用指标明确显示 `unavailable`。预算超限只产生可诊断状态，不自动截断或改变业务请求。
...
分层：CLI 层负责参数和 JSON 契约；账本层负责测量质量、累计、峰值和预算状态；handoff 层负责字段完整性、未闭合工具调用保护、私有原子写入；运行适配层通过受控文件引用消费 handoff，不把账本当作 prompt 正文。
