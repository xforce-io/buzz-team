...
[Issue #46](https://github.com/xforce-io/buzz-team/issues/46)。原 `doctor` 只用 `desktop.selected_rows`，会跳过空公钥行，因此旧 `grok-acp-wrapper` 启动行曾与本机实例身份并存而仍报通过。13 处旧 wrapper 命令已清除：5 条废弃身份行删除，8 条必需 persona 定义保留并清空命令；备份在实例目录。
...
非目标：删除必需的 persona 定义及其他实例行；按同名合并公钥；在 doctor 里改写库存文件。
...
原库存 13 处旧 wrapper 启动命令已清除：5 条废弃身份行删除，8 条必需 persona 定义保留并清空旧启动命令。实际删除后发现误删 5 条必需定义会让身份无法启动，已从备份恢复。当前库存为 9 条本实例身份、12 条 persona 定义、2 条其他实例身份；后 14 条不删除。L1/L2 与修订后的 S1 均已由用户在 2026-09-25 确认。
