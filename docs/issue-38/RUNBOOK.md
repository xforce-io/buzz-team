...
| A15 | TEAM seat processes are identifiable in `ps -E` by **pubkey + `BUZZ_RELAY_URL`** (not by pid-file kill/`pgrep -P`) | **CONFIRMED** read-only observation Quill 2026-09-24 ~15:40 CST (Hogan may cross-check): all 9 TEAM wrappers (`Python -m buzz_team.cli … launch harness`) and their `buzz-acp` children expose seat pubkey in env (`BUZZ_RUNTIME_ID=${ID_TEAM}/<pubkey>`, `GROK_ACP_CWD=…/identities/${ID_TEAM}/<pubkey>/workspace`) and `BUZZ_RELAY_URL=ws://127.0.0.1:3000`. Full `TEAM` hex is **not** present in env/cmdline. Distinguisher vs yuanbao duplicates that may share the same pubkeys: require `BUZZ_RELAY_URL=ws://127.0.0.1:3000` (also observed: `BUZZ_TEAM_INSTANCE=/Users/xupeng/lab/buzz`, `BUZZ_MANAGED_AGENT=xyz.block.buzz.app`; private KEY/SECRET/TOKEN vars present by name only — values not recorded). Pubkeys are **not** visible in `ps` without `-E` (cmdline-only). |
...
| M14 | Mutate/rollback require Desktop **not** running (`require_desktop_not_running`; matcher: `Buzz.app/Contents/MacOS` or exact `Buzz`) **and** no TEAM seat still alive by **identity scan** (pubkey from `*__${TEAM}.json` names + `BUZZ_RELAY_URL` + `buzz_team.cli`/`buzz-acp` argv0; see **A15**). **No** pid-file `kill -0` / `pgrep -P`. **No** Desktop-check escape hatch | In code: `common.sh`; documented here |
