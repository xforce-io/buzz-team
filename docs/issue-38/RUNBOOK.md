...
| A7 | Timing: Desktop reopen → pid files → ACP ready | **CONFIRMED (timing only)** 14:54 sample: whole-app reopen start 14:54:30, pid files written 14:54:38 (~8s). Agent pool is lazy — init only on first message (14:58:06→14:58:17, ~11s). Second sample: terminal reopen 16:13:53; all 9 wrappers + 9 `buzz-acp` processes were up and all 9 pid files rewritten with new pids 27918–27976 ~12s later; seats logged relay connect/online at 16:14:03. Ability to reply text **NOT** yet measured (Hogan's CLI send lacked key). |
...
| A12 | Restart may be whole-app (Cmd+Q + reopen) rather than single-seat; then all 9 pids change | **CONFIRMED** observed 2026-09-24 14:54 (all 9 pid files rewritten 14:54:38), and again in the 16:13:53 reopen (all 9 pids changed, consistent with `--restart-mode app`). Default live flow uses `--restart-mode app`. |
| A13 | Desktop process env must inherit a **listening** system proxy; otherwise doctor reports `proxy_contrast` and seats cannot reply | **FIXED 2026-09-24 16:13–16:14 CST by Hogan (peng-approved option 1).** Pre 16:12:53, all 18 seat processes + Desktop main had `HTTP(S)_PROXY=6478`; doctor `ok:false` with `proxy_contrast` only. LaunchAgent `com.user.proxy-env` was reloaded (plist backed up; disk already 9567, loaded job was 6478): after bootout/bootstrap, 9567×10 and 6478×0; `launchctl setenv` set all six proxy vars to 9567. Hogan quit via `osascript` at 16:13:44 (rc 0, no kill); Desktop + 18 seat processes were gone in 2s and 0 pid files remained. At 16:13:53, Hogan reopened with the exact six-var terminal command below (no `-n`): `X=http://127.0.0.1:9567; open -a Buzz --env HTTP_PROXY=$X --env HTTPS_PROXY=$X --env ALL_PROXY=$X --env http_proxy=$X --env https_proxy=$X --env all_proxy=$X`. Post-reopen Desktop main (26396) + 18 seat processes had all six env keys set to `http://127.0.0.1:9567`, with 0×6478; doctor read all six keys as 9567 across 18 seat processes and returned `ok:true`, `errors=[]`, with `proxy_contrast` passing. A seat text-reply was **not** verified at writing time (Hogan's CLI send lacked key). Evidence: `~/lab/buzz/evidence/proxy-fix-9567-20260924/` (pre/, quit/, post/, plist backup). Future Dock launches should now inherit 9567 after the LaunchAgent reload, but the RUNBOOK retains the terminal `open --env` method for determinism. |
| A14 | After Cmd+Q, relay `ws://127.0.0.1:3000` stays up | **CONFIRMED (process chain + during-quit measurement)** by Hogan 2026-09-24: only listener on `:3000` is pid 76585 `ssh: ~/.colima/_lima/colima/ssh.sock [mux]`, ppid=1 (launchd), started 9/16 22:38; relay runs in colima VM with ssh port-forward, **not** in Desktop (99050) process chain; its start time was unchanged across the 14:54 whole-app reopen. During quit at 16:13:46 CST, Desktop was quitting and `:3000` was still listening via ssh 76585 (colima); `:4500` node 78862 was unchanged. Evidence `~/lab/buzz/evidence/issue-38-live-reverify/a14-relay-3000-20260924-1536/` plus `~/lab/buzz/evidence/proxy-fix-9567-20260924/quit/`. **Notes:** relay depends on colima running; after logout/reboot colima auto-start **NOT** verified — confirm `:3000` listening before running anything (`lsof -nP -iTCP:3000 -sTCP:LISTEN`; scripts abort via `require_relay_3000_listening`). |
...
**PENDING after 16:14 sample:** A8 (prompt-read timing), A11 (MA file vs memory on Desktop rewrite). A13 is fixed; the seat text-reply remains unverified.
...
2. **peng: quit Buzz without killing:** `osascript -e 'quit app "Buzz"'` (equivalent to Cmd+Q), or peng presses Cmd+Q. **Never kill / kill -9.** If quit fails, stop and ask peng to Cmd+Q (all 9 ACP pids should go away).
...
1. **peng: quit Buzz without killing:** `osascript -e 'quit app "Buzz"'` (equivalent to Cmd+Q), or peng presses Cmd+Q. **Never kill / kill -9.** If quit fails, stop and ask peng to Cmd+Q.
2. `ISSUE38_I_UNDERSTAND_LIVE=yes docs/issue-38/scripts/rollback.sh --backup <backup-dir>` (requires Desktop down).
3. **peng reopens from a terminal using the exact approved method (no `-n`):**
   ```bash
   ...
   ```
4. Same proxy + doctor + MA read-back gates: Desktop main + all 18 seat processes have all six proxy env keys set to `http://127.0.0.1:9567`; doctor ok with no `proxy_contrast` (expect `effort=low` `idle=1500` `max_turn=7200`).
