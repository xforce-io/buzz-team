...
闲聊不开工具。被 @ 的交付动作必须 buzz send（在同一频道/私聊发出可见结果）；短答可用当前对话，但交付不得只 👀 或只嘴上说完。
...
- 禁止 `find ~`（及等价全盘扫描）。需要文件时只查已钉路径：`/Users/xupeng/lab/buzz-team/team/prompts/light-keel.md`、`/Users/xupeng/lab/buzz-team/team/prompts/pj.md`、本身份 workspace、当前任务 worktree。
- 状态类提问必须三栏回复：已完成 / 未完成 / 不在本范围；禁止单句笼统「已完成」。
