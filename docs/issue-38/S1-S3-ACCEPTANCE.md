...
| S2 | doctor ok；运行时 effort 与 idle 的理由明确 | 现行 `IDLE-DECISION.md`：不采用 1500→180，周衡 idle 保持 1500。**未**写 managed-agents.json | apply 可改 workflow / prompt / effort；读回 idle 仍是 1500，不是 180 |
