...
当前为 0.1.0 迁移候选。指定本机二进制组合已通过客户端迁移预览；尚未通过最终审查与人工发布门禁，未正式发布。验证候选与证据记录在兼容清单中，后续候选仍须按门禁重新验收。
...
版本事实源：[机器可读兼容清单](src/buzz_team/compatibility.json)。当前观察到的基线为 Buzz Desktop **0.5.23**、Grok **1.0.34 (3736acbc8658)**；定制 buzz-acp 没有版本输出，使用清单中的 SHA-256 标识。其源码 commit 对应关系尚未证实，不以当前检出 SHA 替代。**该组合客户端迁移预览已通过，尚未发布。** 此结论仅覆盖清单中记录的验证候选和范围，不自动认证任意新候选或其他客户端版本。
