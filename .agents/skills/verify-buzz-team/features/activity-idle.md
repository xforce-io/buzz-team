...
测试身份退出直接 relay 成员，只走 NIP-OA 委托，然后对目标频道各完成一次读取和一条回复。成功后把该项标为已规避。未改成员名单时保持未执行，S1 为 fail。2026-09-25 沈予的 `relay_members` 记录在备份后移除，计数持续为 0；Desktop 重启后仍凭 `BUZZ_AUTH_TAG` 登记 owner，订阅炼丹房，并在原线程回帖。
