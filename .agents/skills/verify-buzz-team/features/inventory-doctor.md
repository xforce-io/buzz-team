...
在隔离库存副本追加一条相同公钥与 relay 的启动行后，`health.run(..., depth="doctor")` 的 `ok` 为 false，且出现一条 `inventory_duplicate:*`。写回原字节后 `ok` 为 true，文件字节与追加前一致。测试使用 writer 策略，这样没有 Seatbelt 的平台也不会因为座位沙箱把干净库存判失败。
