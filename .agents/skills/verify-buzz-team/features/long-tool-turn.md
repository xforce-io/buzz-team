...
1. **S1**：后台 / `[bg]` 长工具仍在跑时，不得出现 `session/prompt` 的 `stopReason: end_turn`（或等价 turn close，含 `_x.ai/session/prompt_complete`、`session/update` `turn_completed`、以及 live grok 的 `_x.ai/session_notification` + `turn_completed` / `stop_reason: end_turn`）。假 ACP 夹具：先发 `[bg]` 再立即 `end_turn`，gate 必须等到 PID 退出才放行。
