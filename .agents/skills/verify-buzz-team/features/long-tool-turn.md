...
1. **S1**：后台 / `[bg]` 长工具仍在跑时，不得出现 `session/prompt` 的 `stopReason: end_turn`（或等价 turn close）。假 ACP 夹具：先发 `[bg]` 再立即 `end_turn`，gate 必须等到 PID 退出才放行。
2. **S2**：杀死该 PID 或超过 `timeout_seconds`（默认 1200）后，在 `poll_seconds`（默认 60，≤120）内 stderr 出现 `type=long_tool_alert`（`timeout|exited`）；有 wake 频道上下文时回帖 `【长工具】`。不得再出现无人说话的多小时空窗。
